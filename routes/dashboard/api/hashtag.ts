...
import { createHashtag, deleteHashtag } from "@/utils/hashtags.ts";
...
export const handler: Handlers<null, DashboardState> = {
  async POST(request, ctx) {
    try {
      ...
      await createHashtag(ctx.state.supabaseClient, hashtag);
  ...
  async DELETE(request, ctx) {
    try {
      ...
      await deleteHashtag(ctx.state.supabaseClient, id);
